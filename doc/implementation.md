...
**`version_flag` integration test must use `env!("CARGO_PKG_VERSION")`.** The Phase 10 close caught a hardcoded `"0.1.0"` substring assertion in `crates/llm-cli/tests/integration.rs::version_flag` that broke the moment `bump-version.sh` ran. Fix is to compare against the test crate's `CARGO_PKG_VERSION` at compile time so the assertion follows the workspace version automatically. Lesson: anything that hardcodes the version string outside the four files `bump-version.sh` rewrites needs the same treatment, or the bump script needs to learn about it.
