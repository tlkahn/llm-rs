...
| 10 --- Wrap later-stage features into `llm-wasm` + `llm-python` | v0.10 | Complete | **A**: tools, multi-turn `Conversation`, structured output + schema DSL, built-in tools (`llm_version`, `llm_time`) in both bindings. **B**: chain loop wrapper with per-iteration events, retry/backoff (`RetryProvider`), token budgets, `budget_exhausted` surfaced on `ChainResult`. **C**: `ConversationStore` trait in `llm-store` (wasm-safe) with `LogStore` impl; `llm-python` `LogStore` pyclass + `Conversation.load`/`persist_to` + auto-logging + `AgentConfig`/`AgentConfig.from_toml` + `LlmClient.run_agent`; `llm-wasm` `JsConversationStore` JS-callback backend + `setConversationStore` + `Conversation.load` + `AgentConfig`/`runAgent` |
