...
/// Provider wrapper that retries transient errors with exponential backoff.
///
/// WASM variant of `llm-cli`'s `RetryProvider` — uses `gloo-timers` for the
/// delay instead of `tokio::time::sleep`, and the `?Send` async trait bound.
